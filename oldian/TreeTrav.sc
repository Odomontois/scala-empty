...
def boundSearch(start: Long, target: Long) =
  search[Set[Long], Long](start, IterDef( x ⇒ s ⇒ List(x * 2, x * 3).filter(!s(_)), _.toSet, _ > target))
...
def backSearch[N: Integral](start: N, target: N): ((Set[N], Boolean), Option[(N, Int)]) = {
  ...
  val two = N.fromInt(2)
  def gen(x: N, depth: Int)(seen: Set[N]) = List(
    (x % two == N.zero, x / two),
    (x > N.one, x - N.one),
    (x > two, x - two)
  ).collect { case (true, y) if !seen(y) ⇒ (y, depth + 1) }
  search[Set[N], (N, Int)](
    (start, 0),
    IterDef(
      (gen _).tupled,
...
def forwardSearch[N: Integral](start: N, target: N): ((Set[N], Boolean), Option[(N, List[N])]) = {
  val N = implicitly[Numeric[N]]
  val two = N.fromInt(2)
  def gen(x: N, way: List[N])(seen: Set[N]) = List(
    (x % two == N.zero, x / two),
    (x > N.one, x - N.one),
    (x > two, x - two)
  ).collect { case (true, y) if !seen(y) ⇒ (y, x :: way) }
